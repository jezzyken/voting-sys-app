  ...
  <div>
    <v-container>
      ...
      <v-card class="mb-6 primary" dark>
        <v-card-text class="text-center">
          ...
          <p class="subtitle-1 mb-0">Election: {{ electionName }}</p>
      ...
      <template v-else>
        ...
        >
          ...
          <v-card-subtitle
            >Select one candidate for this position</v-card-subtitle
          >
    ...
    <v-dialog v-model="summaryDialog" max-width="500px" persistent>
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn text @click="summaryDialog = false" :disabled="submitting">
            Review Again
          </v-btn>
          <v-btn
            color="primary"
...
export default {
  ...
  computed: {
    ...
    isVoteComplete() {
      return (
        ...
        this.selectedPositionsCount === this.totalPositions
      );
    },
  ...
  async created() {
    ...
    await Promise.all([this.fetchElectionDetails(), this.checkVoteStatus()]);
